...
run_test() {
    $ECHO "$2" | $AZCC > test/hsjoihs_tmp/task$1.s 2>| /dev/null
    d=$?
    if [ $d -ne 0 ]; then { $ECHO "compile FAIL, at test case" $1: "$2"; return 0; }; else $ECHO "\033[32mcompile PASS\033[m"; fi
    gcc test/hsjoihs_tmp/task$1.s -o test/hsjoihs_tmp/task$1.out -static
	./test/hsjoihs_tmp/task$1.out
	res=$?
	if [ $res -ne $3 ]; then { $ECHO "got:" $res; echo "expected:" $3; $ECHO "\033[31mFAIL\033[m, at test case" $1: "$2"; }; else $ECHO "\033[32mPASS\033[m"; fi
}
