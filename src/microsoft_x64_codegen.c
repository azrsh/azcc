...
static const char *argumentRegister64[4] = {"rcx", "rdx", "r8", "r9"};
// static const char *argumentRegister32[4] = {"ecx", "edx", "r8d", "r9d"};
// static const char *argumentRegister16[4] = {"cx", "dx", "r8w", "r9w"};
// static const char *argumentRegister8[4] = {"cl", "dl", "r8b", "r9b"};
  ...
  *labelCount += 1;
  ...
  //定義された変数の分の領域を確保
  {
    ...
    printf("  mov rbp, rsp\n");
    {
      ...
      if (type_to_stack_size(definition->returnType) > 1)
        stackSize += 8;
      printf("  sub rsp, %zu\n", stackSize);
    }
  ...
  //引数の代入処理
  {
    ...
    if (type_to_stack_size(definition->returnType) > 1 * 8) {
