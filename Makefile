...
test/unit/ccaz1/%.out: self $(filter-out main.o $(TEST_SELF_OBJ_NAMES), $(OBJS)) $(TEST_TOOL_OBJS) test/unit/%.c
	./test/unit/rmlink.sh
	$(CC) -c -I test/self -o test/unit/ccaz1/$*.o test/unit/$*.c
	$(CC) -o $@ test/unit/ccaz1/$*.o $(filter-out main.o $(TEST_SELF_OBJ_NAMES), $(OBJS)) $(TEST_SELF_OBJS) $(TEST_TOOL_OBJS)
